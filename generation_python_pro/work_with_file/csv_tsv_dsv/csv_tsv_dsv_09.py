...
# имена всех пассажиров мужского пола, а затем — женского, имена же непосредственно
...
with open('titanic.csv', encoding='UTF-8') as file:
    ...
    next(data)
    for row in data:
        sur, name, sex, age = row
        if sur == '1' and float(age) < 18:
            dct[sex] = dct.get(sex, []) + [name]

    print(*dct['male'], sep='\n')
    print(*dct['female'], sep='\n')
