# Вам доступен файл student_counts.csv, который содержит данные о количестве
# учеников в некотором учебном заведении за период 2000 — 2021 г.
# В первом столбце записан год, в последующих столбцах записан класс и
# количество учеников в данном классе в этом году:
#
...
# Примечание 2. Разделителем в файле student_counts.csv является запятая,
# при этом кавычки не используются.

import csv

with open('student_counts.csv', encoding='UTF-8') as inside, open('sorted_student_counts.csv', 'w', encoding='UTF-8', newline='') as outside:
    d = {}
    data = csv.DictReader(inside)

    column = data.fieldnames[:1] + (sorted(data.fieldnames[1:], key=lambda x: (len(x), x)))

    new_data = (list(data))

    writer = csv.DictWriter(outside, fieldnames=column)
    writer.writeheader()
    for row in new_data:
        writer.writerow(row)
