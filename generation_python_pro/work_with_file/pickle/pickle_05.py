# По каналу связи передается pickle файл, содержащий сериализованный словарь или список,
...
# Программа должна вычислить контрольную сумму для объекта,
# который содержится в данном pickle файле, и если она совпадает с введенным числом, вывести текст:
...
name, contrl_sum = input(), int(input())
answers = ['Контрольные суммы не совпадают', 'Контрольные суммы совпадают']

with open(name, 'rb') as file:
    obj = pickle.load(file)
    lst = list(filter(lambda x: isinstance(x, int), obj)) or [0]
    check = sum(lst) if isinstance(obj, dict) else max(lst) * min(lst)
    print(answers[check == contrl_sum])
