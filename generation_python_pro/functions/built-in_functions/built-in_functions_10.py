...
# каждый из которых является списком, и один необязательный именованный аргумент fill,
# имеющий значение по умолчанию None.
#
# Функция должна объединять элементы переданных последовательностей в кортежи,
# налогично функции zip(), и возвращать в виде списка,
# но если последовательности имеют различную длину, недостающие элементы
# последовательностей меньшей длины должны принимать значение fill.
#
...
# 5, второй —
# 3
# 3, то есть элементам 4 и 5 из первого списка нет пар из второго списка.
# В таком случае, функция должна сопоставить им значение fill, равное '_'.
# Итак, результатом работы функции будет список:
#
# [(1, 'a'), (2, 'b'), (3, 'c'), (4, '_'), (5, '_')]
...
def zip_longest(*args, fill=None):
    lelens = max(map(len, args))
    [i.extend([fill] * (lelens - len(i))) for i in args]
