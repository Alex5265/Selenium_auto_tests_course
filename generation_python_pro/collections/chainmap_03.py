...
# указано название ингредиента, а в качестве значения — его цена.
# Все ингредиенты разбиты по категориям, например, овощи содержатся в одном словаре,
# соусы — в другом.
#
...
# На вход программе подается последовательность ингредиентов,
# разделенных запятой без пробелов.
#
# Формат выходных данных
# Программа должна определить общую стоимость введенных ингредиентов
# и вывести полученный результат в виде чека, в котором указаны ингредиенты
# в лексикографическом порядке, количество каждых и их общая стоимость, в следующем формате:
#
...
#
# Примечание 2. Длина пунктирной линии должна равняться длине самой длинной
# строки в чеке, включая строку с итоговой стоимостью.
#
# Примечание 3. Гарантируется, что все ингредиенты, выбранные
# посетителем, присутствуют в словарях.
...
order = Counter(list(input().split(',')))
