...
#
# Вам доступен список tasks, в котором записаны все дела Тимура.
# Каждый элемент списка представляет собой кортеж из трех элементов:
...
# указав для каждого набор соответствующих действий в правильной очередности,
# в следующем формате:
#
# <дело>:
#     1. <действие>
...
group_task = groupby(sorted(tasks, key=lambda tpl: tpl[0]), key=lambda tpl: tpl[0])

for task, group in group_task:
    print(f'{task}:')
    res = [(i[-1], i[-2]) for i in group]
    for i in sorted(res):
        print(f'    {i[0]}. {i[-1]}')
    print()
